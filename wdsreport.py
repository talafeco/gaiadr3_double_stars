...
for fitsFile in files:
    ...
    if args.find_doubles_on_image:
        ...
        print('\n### Doubel stars should be on the image ###')
        ...
        if args.generate_wds_list:
            doubles_on_image_table.write(fitsFileName[:-4] + '_wds_pairs.csv', format='ascii', overwrite=True, delimiter=',')

    sources_ds = dscalculation.get_ds_from_image(sources_ds, wds_catalog, fits_data, fits_header, fitsFileName, fits_file_date, image_wcs, wdsTable, dao_sigma, dao_fwhm, dao_threshold, search_cone)
